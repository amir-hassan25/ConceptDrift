...
class TGNMemory(torch.nn.Module):
    def __init__(self, num_nodes: int, raw_msg_dim: int, memory_dim: int,
                 time_dim: int, node_features: Tensor, message_module: Callable,
                 aggregator_module: Callable, preserve_semantics: bool, baseline: bool):
        super().__init__()
    ...
    def reset_state(self):
        if self.baseline == True:
            print("Initializing with baseline memory")
            zeros(self.memory)
        else:
            print("Initializing with semantic memory")
            self.memory = self.node_features.clone()
    ...
    def _get_updated_memory(self, n_id: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # Check if the aggregator requires node memory (for CrossAttentionAggregator)
        if isinstance(self.aggr_module, CrossAttentionAggregator):
...
class IdentityMessage(torch.nn.Module):
    def __init__(self, raw_msg_dim: int, memory_dim: int, time_dim: int):
        ...
        # only if self.preserve_semantics is true
        self.out_channels = raw_msg_dim + 2 * memory_dim + time_dim 
